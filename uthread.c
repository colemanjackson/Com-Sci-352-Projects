...
//semaphore that makes sure running kernel threads are less than max
kernel threads
sem_t thread_lock;
...
{
    //initialize this semaphore with the value of max_number_of_klt so
    that it will keep track of how many threads are currently running and
    if we are allowed to start anymore running
        sem_init(&thread_lock, 0, max_number_of_klt);
    //initialize this semaphore with the value 1, because we only want 1
    thread at a time to access the queue
...
int uthread_create(void (*func)())
{
    ...
    //create a process that will run func when it is called with
...
void uthread_yield()
{
    if(pq.size()>0)
    {
        ...
        pq.pop();
        sem_post(&queue_lock);
        //create a context for the current thread MAY NEED TO CHANGE THE NAME
        OF PROCESS BECAUSE IT IS USED ABOVE
        ucontext_t *contxt;
...
{
    //run the user thread if there are enough kernel threads left
    (currently running - max running)
